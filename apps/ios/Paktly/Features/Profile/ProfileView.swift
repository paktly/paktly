        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    PaktlyPanel {
                        DisclosureGroup("Edit profile", isExpanded: $editingProfile) {
                        VStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 14) {
                            ...
                            HStack(spacing: 12) {
                                Image(systemName: model.currentUser?.smartAccount == nil ? "envelope.badge.shield.half.filled" : "person.badge.key.fill")
                                    ...
                                    .background(PaktlyColor.mint.opacity(0.35), in: Circle())
                                VStack(alignment: .leading, spacing: 3) {
                                    Text(model.currentUser?.smartAccount == nil ? "Account security" : "Wallet passkey active")
                                        .font(.subheadline.weight(.semibold))
                                    Text(model.currentUser?.smartAccount == nil
                                         ? "Keep access to the sign-in method you use for Paktly."
