...
Ordinary onboarding uses email OTP and creates no wallet. Existing wallet users can still choose passkey sign-in. A signed-in user activates Paktly Smart from the profile when they want stored-value features; only that action invokes SocketFi's native passkey flow and links the resulting smart account.
