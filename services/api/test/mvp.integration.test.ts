...
import { NotificationDeliveryWorker } from "../src/modules/notifications/worker.js";
import type { PushMessage, PushNotificationProvider } from "../src/modules/notifications/apns-provider.js";
...
suite("expense-sharing MVP end to end", () => {
  ...
  it("creates profiles, plans, invitations and memberships", async () => {
    ...
    sessions.set("eve", { token: eveSession.json().accessToken, id: eveSession.json().user.id });
    const eveInbox = await app.inject({ method: "GET", url: "/api/v1/invitations", headers: auth("eve") });
    expect(eveInbox.json().invitations).toHaveLength(1);
  ...
  it("calculates balances, simplifies debt, records settlement, activity and notifications", async () => {
    ...
    expect(notifications.json().unreadCount).toBeGreaterThan(0);
    const installationId = randomUUID();
    const registered = await app.inject({ method: "POST", url: "/api/v1/devices/push", headers: auth("bob"), payload: { installationId, token: "ab".repeat(32), environment: "SANDBOX", locale: "en-US", timezone: "UTC", appVersion: "0.1.0" } });
    expect(registered.statusCode).toBe(201);
    const [deliveries] = await app.db`SELECT count(*)::int count FROM notification_deliveries d JOIN push_devices pd ON pd.id=d.device_id WHERE pd.installation_id=${installationId}`;
    expect(Number(deliveries?.count)).toBeGreaterThan(0);
    const sent: PushMessage[] = [];
    const provider: PushNotificationProvider = { send: (message) => { sent.push(message); return Promise.resolve({ status: 200, apnsId: randomUUID() }); } };
    const processed = await new NotificationDeliveryWorker(app.db, provider).runBatch(100);
    expect(processed).toBeGreaterThan(0);
    expect(sent.length).toBe(processed);
    expect(sent[0]?.data.notificationId).toBeTruthy();
    const preferences = await app.inject({ method: "PATCH", url: "/api/v1/notification-preferences", headers: auth("bob"), payload: { expenses: false, lockScreenDetail: "PRIVATE" } });
    expect(preferences.json().preferences.expenses).toBe(false);
    expect(preferences.json().preferences.lock_screen_detail).toBe("PRIVATE");
  });
});
